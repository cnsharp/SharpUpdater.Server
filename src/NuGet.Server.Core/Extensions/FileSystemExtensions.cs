...
namespace CnSharp.Updater.Server.Extensions
{
    /// <summary>
    /// Extract from https://github.com/NuGet/NuGet2/blob/main/src/Core/Extensions/FileSystemExtensions.cs
    /// </summary>
    public static class FileSystemExtensions
    {
        ...
        public static void DeleteFiles(IFileSystem fileSystem, IEnumerable<IPackageFile> files, string rootDir)
        {
        ...
        public static IEnumerable<string> GetDirectoriesSafe(this IFileSystem fileSystem, string path)
        {
            try
            {
        ...
        public static IEnumerable<string> GetFilesSafe(this IFileSystem fileSystem, string path)
        {
        ...
        [SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes",
            Justification = "We want to log an exception as a warning and move on")]
        public static IEnumerable<string> GetFilesSafe(this IFileSystem fileSystem, string path, string filter)
        {
            try
            {
        ...
        public static void DeleteDirectorySafe(this IFileSystem fileSystem, string path, bool recursive)
        {
            DoSafeAction(() => fileSystem.DeleteDirectory(path, recursive), fileSystem.Logger);
        }

        public static void DeleteFileSafe(this IFileSystem fileSystem, string path)
        {
        ...
        public static void DeleteFileAndParentDirectoriesIfEmpty(this IFileSystem fileSystem, string filePath)
        {
            ...
            for (string path = Path.GetDirectoryName(filePath);
        ...
        public static void AddFileWithCheck(this IFileSystem fileSystem, string path, Func<Stream> streamFactory)
        {
            if (fileSystem.FileExists(path))
            {
        ...
        [SuppressMessage("Microsoft.Reliability", "CA2000:Dispose objects before losing scope",
            Justification = "The caller is responsible for closing the stream")]
        public static void AddFileWithCheck(this IFileSystem fileSystem, string path, Action<Stream> write)
        {
            if (fileSystem.FileExists(path))
            {
                fileSystem.Logger.Log(MessageLevel.Warning, NuGetResources.Warning_FileAlreadyExists, path);
