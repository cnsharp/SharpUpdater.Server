...
{
    /// <summary>
    /// SharpUpdaterServerPackageRepository represents a folder of sps on disk. All packages are cached during the first request
    ...
    /// </summary>
    public class SharpUpdaterServerPackageRepository
        : IServerPackageRepository, IDisposable
    {
        ...
        public SharpUpdaterServerPackageRepository(
            string path,
            IHashProvider hashProvider,
            ISettingsProvider settingsProvider = null,
            NuGet.Server.Core.Logging.ILogger logger = null)
        {
            ...
            _serverPackageStore = new SharpUpdaterServerPackageStore(
                _fileSystem,
                new SharpUpdaterExpandedPackageRepository(_fileSystem, hashProvider),
                _logger);
        }

        internal SharpUpdaterServerPackageRepository(
            IFileSystem fileSystem,
            bool runBackgroundTasks,
            SharpUpdaterExpandedPackageRepository innerRepository,
            ISettingsProvider settingsProvider = null,
            NuGet.Server.Core.Logging.ILogger logger = null)
        {
        ...
        private void AddPackagesFromDropFolderWithoutLocking()
        {
            ...
            try
            {
                ...
                foreach (var packageFile in _fileSystem.GetFiles(_fileSystem.Root, "*" + Constants.PackageExtension, recursive: false))
                {
                    try
                    {
                        // Create package
                        var package = SharpUpdaterPackageFactory.Open(_fileSystem.GetFullPath(packageFile));
        ...
        private async void FileSystemChangedAsync(object sender, FileSystemEventArgs e)
        {
            try
            {
                ...
                // 1) If a .sp is dropped in the root, add it as a package
                if (string.Equals(changedDirectory, _watchDirectory, StringComparison.OrdinalIgnoreCase)
                    && string.Equals(Path.GetExtension(e.Name), Constants.PackageExtension, StringComparison.OrdinalIgnoreCase))
                {
                ...
                {
                    // TODO: invalidating *all* packages for every sp change under this folder seems more expensive than it should.
                    // Recommend using e.FullPath to figure out which sps need to be (re)computed.
        ...
        {
            private readonly SharpUpdaterServerPackageRepository _repository;
            private Lock _lockHandle;

            public SuppressedFileSystemWatcher(SharpUpdaterServerPackageRepository repository)
            {
