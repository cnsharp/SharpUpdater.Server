// Copyright (c) CnSharp Studio. All rights reserved.
// Licensed under the Apache License, Version 2.0. See License.txt in the project root for license information.
// Modified from NuGet.OptimizedZipPackage
...
{
    /// <summary>
    /// Represents a NuGet package backed by a .sp file on disk.
    /// </summary>
    /// <remarks>
    /// Unlike <see cref="SharpPackage"/>, OptimizedZipPackage doesn't store content files in memory.
    /// Instead, it unzips the .sp file to a temp folder on disk, which helps reduce overall memory usage.
    /// </remarks>
    public class OptimizedZipPackage : SharpPackage
    {
        // The DateTimeOffset entry stores the LastModifiedTime of the original .sp file that
        ...
        private static readonly IFileSystem _tempFileSystem = new PhysicalFileSystem(
            Path.Combine(Path.GetTempPath(), "SharpScratch", Guid.NewGuid().ToString()));
        ...
        public OptimizedZipPackage(string fullPackagePath) : base(fullPackagePath)
        {
        ...
        /// Initializes a new instance of the <see cref="OptimizedZipPackage" /> class.
        /// </summary>
        /// <param name="fileSystem">The file system which contains the .sp file.</param>
        ...
        private void EnsureManifest()
        {
            ...
            {
                using (Stream manifestStream = stream.GetManifestStream())
                {
        ...
        private void EnsurePackageFiles()
        {
            ...
            using (Stream stream = GetStream())
            {
                ...
                var packageId = SharpPackage.GetPackageIdentifier(package);
                // unzip files inside package
                var files = from part in package.GetParts()
                            where SharpPackage.IsPackageFile(part, packageId)
                            select part;
            ...
            lock (_cachedExpandedFolder)
            {
                if (_cachedExpandedFolder.Count > 0)
                {
                    foreach (var valueTuple in _cachedExpandedFolder.Values)
                    {
                        ...
                        Infrastructure.FileSystemExtensions.DeleteDirectorySafe(_tempFileSystem, expandedFolder, true);
                    }

                    _cachedExpandedFolder.Clear();
                }
                Infrastructure.FileSystemExtensions.DeleteDirectorySafe(_tempFileSystem, _tempFileSystem.Root, true);
            }
        }
    }
}