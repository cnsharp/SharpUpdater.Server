...
namespace CnSharp.Updater.Server.Infrastructure
{
    ...
    {
        private const string TemplateNupkgFilename = "{0}\\{1}\\{0}.{1}{2}";
        private const string TemplateHashFilename = "{0}\\{1}\\{0}.{1}{2}";

        private readonly object _syncLock = new object();

        private readonly IFileSystem _fileSystem;
        private readonly PackageRepositoryBase _expandedPackageRepository;
        ...
        public ServerPackageRepository(string path, IHashProvider hashProvider, Logging.ILogger logger)
        {
            ...
            _expandedPackageRepository = new SharpExpandedPackageRepository(_fileSystem, hashProvider);//modified
        ...
        private void AddPackagesFromDropFolder()
        {
            ...
            try
            {
                ...
                foreach (var packageFile in _fileSystem.GetFiles(_fileSystem.Root, "*"+Strings.PackageExt, false))
                {
                    try
                    {
                        // Create package
                        var package = new SharpPackage(_fileSystem.OpenFile(packageFile));
        ...
        private void FileSystemChanged(object sender, FileSystemEventArgs e)
        {
            ...
            if (String.Equals(Path.GetDirectoryName(e.FullPath), _fileSystemWatcher.Path, StringComparison.OrdinalIgnoreCase)
                && String.Equals(Path.GetExtension(e.Name), Strings.PackageExt, StringComparison.OrdinalIgnoreCase))
            {
        ...
        {
            return string.Format(TemplateNupkgFilename, packageId, version.ToNormalizedString(),Strings.PackageExt);
        }
