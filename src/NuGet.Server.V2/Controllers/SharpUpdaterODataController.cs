...
namespace CnSharp.Updater.Server.V2.Controllers
{
    [NuGetODataControllerConfiguration]
    public abstract class SharpUpdaterODataController : ODataController
    {
        private const string ApiKeyHeader = Constants.ApiKeyHeader;
        ...
        protected SharpUpdaterODataController(
        ...
        public virtual async Task<HttpResponseMessage> UploadPackage(CancellationToken token)
        {
            ...
            var package = SharpUpdaterPackageFactory.Open(temporaryFile);
            ...
            if (_authenticationService.IsAuthenticated(User, apiKey, package.Id))
            {
                ...
                {
                    retValue = CreateStringResponse(HttpStatusCode.InternalServerError, e.Message);
                }
            }
            else
            {
