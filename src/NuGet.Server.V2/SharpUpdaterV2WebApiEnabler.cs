...
namespace CnSharp.Updater.Server.V2
{
    public static class SharpUpdaterV2WebApiEnabler
    {
        /// <summary>
        /// Enables the SharpUpdater V2 protocol routes on this <see cref="HttpConfiguration"/>. Note that this method does
        /// not activate the legacy push URL, <code>api/v2/package</code>. To activate the legacy push route, use the
        /// <see cref="UseSharpUpdaterV2WebApiFeed(System.Web.Http.HttpConfiguration,string,string,string,bool)"/> method overload.
        /// </summary>
        ...
        public static HttpConfiguration UseSharpUpdaterV2WebApiFeed(this HttpConfiguration config,
            string routeName,
            string routeUrlRoot,
            string oDatacontrollerName)
        {
            return config.UseSharpUpdaterV2WebApiFeed(
                routeName,
                routeUrlRoot,
                oDatacontrollerName,
        ...
        /// <summary>
        /// Enables the SharpUpdater V2 protocol routes on this <see cref="HttpConfiguration"/>.
        /// </summary>
        /// <param name="config">The HTTP configuration associated with your web app.</param>
        /// <param name="routeName">The route name prefix, to allow multiple feeds per web app.</param>
        /// <param name="routeUrlRoot">The base URL for the routes, to allow multiple feeds per web app.</param>
        ...
        {
            var hashProvider = new CryptoHashProvider(NuGet.Server.Core.Constants.HashAlgorithm);
            return new SharpUpdaterServerPackageRepository(packagePath, hashProvider, settingsProvider, logger);          
        }
